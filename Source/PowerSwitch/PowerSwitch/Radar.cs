...
namespace CommonMisc
{
    ...
    public class Radar
    {
        ...
        /// <returns></returns>
        public static IEnumerable<Pawn> FindEnemyPawnsNoAnimals(IntVec3 Position, Map map, float Distance)
        {
            // LINQ version
            return map.mapPawns.AllPawnsSpawned.Where(p => !p.RaceProps.Animal && p.HostileTo(Faction.OfPlayer) && !p.InContainerEnclosed && p.Position.InHorDistOf(Position, Distance));
        }
        /// <summary>
        /// Find enemy pawns in reach and return a list
        /// </summary>
